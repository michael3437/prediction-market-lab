...
A suite for collection and analysis of data from all markets on Kalshi. Measures prediction accuracy using Brier scores across a range of categories of markets, with options for slicing by volume, OI, or liquidity. Will take orderbook snapshots for active markets to be used for future analysis and investigating if orderbook features provide insight into the final outcome probability. Equipped with a full data pipeline storing market data and categorising it, as well as OHLC price history for each market.
